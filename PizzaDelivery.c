...
int main(int argc, char *argv[])
{
    ...
    int i;
    int wait = 0;
    int *id = (int *)malloc(customers * sizeof(int));
    if (id == NULL)
    {
        printf("Out of memory!");
        exit(-1);
    ...
    for (i = 0; i < customers; i++)
    {
        ...
        if (pthread_create(&threads[i], NULL, &simulateServiceFunc, &id[i]) != 0)
        {
        ...
        wait = (rand_r(&newseed) % (T_ORDERHIGH - T_ORDERLOW + 1)) + T_ORDERLOW;
        sleep(wait);
    }
